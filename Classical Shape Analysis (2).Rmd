...
```{r}
ffRV <- function(x)
    ffRV1(x) + ffRV2(x)+ffRV3(x)
ffRC <- function(x)
    ffRC1(x) + ffRC2(x)+ffRC3(x)
```

```{r}

x <- seq(min(cvq[,1,1])-1.3, max(cvq[,1,1])+1.3, length=100)
y <- seq(min(cvq[,2,1])-1.6, max(cvq[,2,1])+1.6, length=100)
xy <- expand.grid(x=x, y=y)
z <- matrix(apply(as.matrix(xy), 1, ffRV), length(x), length(y))

---
title: 'Orang-utan Faceshape (Male vs. Female) Bayesian Procrusts:'
author: "Debashis Chatterjee"
date: "11/11/2020"
output: pdf_document
---
\section{Load the Landmark Data of Orang-utan}
...
\section{Start \& Run MCMC for Orang-utan Male upto 10 objects}
```{r}
require(BPviGM1)
...
\section{Start \& Run MCMC for Orang-utan Male upto 20 objects}
...
\section{Start \& Run MCMC for Orang-utan Male upto 30 objects}
...
\section{Start \& Run MCMC for Orang-utan Female upto 10 objects}
```{r}
...
\section{Start \& Run MCMC for Orang-utan Female upto 20 objects}
...
```
\section{Start \& Run MCMC for Orang-utan Female upto 30 objects}
...
\section{Plot for Posterior Convergence of Orang-utan Male}
...
\section{Plot for Posterior Convergence of Orang-utan Female}
...
\section{Combined-Plot for Shape-Variance Comparison Plot of Orang-utan Female vs. Male}
...
legend("topleft",cex=0.6,title="Orang (Female)", c("n=10","n=20","n=30", expression(("Bayes Estimate of ")~~tilde(sigma))), lty = c(1,1,1,3), col = c("orange","hotpink","red","red4"), lwd = c(1,1,1,1))
abline(v=mean(apem10000.30), col="darkblue",lwd=2, lty=3)
#lines(density(apem10000.20), ylim=c(0,12),col="blue")
lines(density(apem10000.20), ylim=c(0,12),col="blue")
lines(density(apem10000.30), ylim=c(0,12),col="blue4")
lines(density(apem10000.10[1000:10000]), ylim=c(0,12),col="skyblue");
legend("topright",cex=0.6,title="Orang(Male)", c("n=10","n=20","n=30", expression(("Bayes Estimate of ")~~tilde(sigma))), lty = c(1,1,1,3), col = c("skyblue","blue","blue4","darkblue"), lwd = c(1,1,1,1))
grid()
```
